...
function ChatInbox({onClick}){
    ...
    useEffect(() => {
        const fetchInboxMessages = async () => {
            try {
                const response = await fetch(`https://https.we-make-team.click/api/message/last-messages?receiverId=${userId}`);
                if (!response.ok) {
...
const getPhotoUrl = (message, userId) => {
    ...
    const defaultPhotoUrl = `https://https.we-make-team.click/userphotos/randomUser.jpg`;
