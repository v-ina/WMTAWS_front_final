...
function AdminReportPage(){
    ...
    const fetchReports = async() => {
        const token = localStorage.getItem('jwt')
        if(token){
            const responseAfterFetch = await fetch(`http://ec2-${apiUrl}.eu-west-3.compute.amazonaws.com:${apiPort}/api/reports`, {method : "GET", headers : {'Authorization': `Bearer ${token}`}})
    ...
    const handleIgnoreReports = async(event, reportId) => {
        const token = localStorage.getItem('jwt')
        await fetch(`http://ec2-${apiUrl}.eu-west-3.compute.amazonaws.com:${apiPort}/api/reports/${reportId}`, {method : "DELETE", headers : {'Authorization': `Bearer ${token}`}})
        fetchReports()
    }
    ...
    const handleDeleteArticle = async(event, articleId, reportId) => {
        if(window.confirm(`are you sure that want to delete this reported article?`)){
            ...
            const token = localStorage.getItem('jwt')
            await fetch(`http://ec2-${apiUrl}.eu-west-3.compute.amazonaws.com:${apiPort}/api/articles/${articleId}`, {method : "DELETE", headers : {'Authorization': `Bearer ${token}`}})
    ...
    const handleDeleteComment = async(event, commentId, reportId) => {
        if(window.confirm(`are you sure that want to delete this reported comment?`)){
            ...
            const token = localStorage.getItem('jwt')
            await fetch(`http://ec2-${apiUrl}.eu-west-3.compute.amazonaws.com:${apiPort}/api/comments/${commentId}`, {method : "DELETE", headers : {'Authorization': `Bearer ${token}`}})
            handleIgnoreReports(event, reportId)
            fetchReports()
        } else {
            alert('your require was canceled')
