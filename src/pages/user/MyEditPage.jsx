...
function MyEditPage(){
    ...
    const handleSubmitEditAccount = async (event) => {
        ...
        const token = localStorage.getItem("jwt") 
        const EditUserResponse = await fetch(`https://https.we-make-team.click/api/users/${loginUser.id}`, {
            method : "PUT",
            headers : {
    ...
    const handleDeleteMyself = async(event, userId) =>{
        ...
        if(window.confirm(`are you sure that you want to delete your account?`)){
            ...
            const token = localStorage.getItem('jwt')
            await fetch(`https://https.we-make-team.click/api/users/${userId}`, {method : "DELETE", headers : {'Authorization': `Bearer ${token}`}})
            setTimeout(()=>{
        ...
        <Header />
            {loginUser && (
                <main className="mypageEdit--main">
                    <div className='mypageEdit--user__img'><img src={loginUser.photo !== null ? (loginUser.photo) : (`https://https.we-make-team.click/userphotos/randomUser.jpg`)} alt="image utilisateur" /></div>
